...
- **Важно про ресурсы (v1.10.1)**: E2B base-шаблон = **478 МБ RAM, 2 CPU**.
  Полный скан с БД (716 МБ распакована) → **OOM (EXIT=137)** уже на 300 IP/30 потоков.
  Рабочий режим — **аудит списка целей БЕЗ большой БД**:
  ```bash
  # из диспетчера (берёт непроверенные роутеры из БД):
  python3 dispatch.py e2b_probe --batch 10

  # напрямую:
  python3 e2b_audit.py --script e2b_targets_audit.py \
    --args "--targets-file /tmp/t.txt --mode http" --upload /tmp/t.txt
  ```
  `e2b_targets_audit.py` — лёгкий аудитор (socket, без SQLite-БД): HTTP-баннеры
  или TCP-reachability. Файлы целей загружаются в `/tmp/` песочницы (--upload).
