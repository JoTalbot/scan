#!/usr/bin/env python3
"""
Import scan chunks from workers (главная машина)
================================================
Читает data/scans/scan_shard_*.csv.gz (залитые исполнителями) и вливает их
в isp_cidr.db: INSERT OR REPLACE в scan_results + детекция роутеров
(router_detect) + запись в scan_routers.

После импорта — sync_manager.py (архив БД + push).

Usage:
    python3 import_chunks.py [--dry-run] [--delete-after]
"""
import os
import sys
import gzip
        ...
        for r in rows:
            try:
                batch.append((
                    ...
                    int(r["port"]) if r.get("port") else 80,
                    r.get("status", "closed"),
                    int(r.get("has_banner") or 0),
                    int(r["http_status"]) if r.get("http_status") else None,
                    r.get("server_header"), r.get("title"), r.get("realm"),
                    float(r["response_time_ms"]) if r.get("response_time_ms") else None,
                    int(r["asn"]) if r.get("asn") else None,
                    r.get("isp_name"), r.get("country_code"),
                    r.get("country_name_ru"), r.get("region"),
                    r.get("scanned_at", get_now()), r.get("agent_id", "import"),
                ))
                # детекция роутеров по баннерам из чанка (баннер не храним — используем title/server)
                det = detect_router(server_header=r.get("server_header"),
                                    title=r.get("title"), banner=None)
                if det and not dry_run:
                    ...
                    """, (r["ip"], batch[-1][1], batch[-1][2], batch[-1][5],
                          ...
                          batch[-1][10], batch[-1][11], batch[-1][12], batch[-1][13],
                          batch[-1][14], batch[-1][15], get_now(),
                          r.get("agent_id", "import"), "import"))
                    routers_found += 1
...
def main():
    ...
    parser.add_argument("--delete-after", action="store_true", help="Удалить чанки после импорта")
    ...
    if args.files:
        ...
        files = sorted(glob.glob(os.path.join(BASE_DIR, "data", "scans", "scan_shard_*.csv.gz")))
    if not files:
    ...
    print(f"\n✅ Импортировано: {imported} записей, роутеров: {routers}"
