...
def record_shard_completed(job_id, shard_id):
    return _job_state().mark_shard_completed(
        _safe_ref(job_id, "job_id"), shard_id, state_path=JOB_STATE_FILE
    )


def shard_completed(job_id, shard_id):
    return _job_state().shard_completed(
        _safe_ref(job_id, "job_id"), shard_id, state_path=JOB_STATE_FILE
    )
...
def lock_task(agent_id, task_name, step, machine_id="Machine-01"):
    ...
    active.append({"agent_id": agent_id, "machine_id": machine_id, "task_name": task_name,
                   "current_step": step, "status": "IN_PROGRESS", "started_at": get_now_iso(),
...
def update_step(agent_id, step):
    ...
    if not found:
        state.setdefault("active_agents", []).append({"agent_id": agent_id, "machine_id": "Unknown",
            "task_name": "Task in progress", "current_step": step, "status": "IN_PROGRESS",
            "started_at": get_now_iso(), "updated_at": get_now_iso()})
    save_state(state); update_status_markdown(state)
...
def main():
    ...
    p_lock = subparsers.add_parser("lock"); p_lock.add_argument("--agent", required=True); p_lock.add_argument("--task", required=True); p_lock.add_argument("--step", required=True); p_lock.add_argument("--machine", default="Sandbox-01")
    p_step = subparsers.add_parser("step"); p_step.add_argument("--agent", required=True); p_step.add_argument("--step", required=True)
    p_done = subparsers.add_parser("complete"); p_done.add_argument("--agent", required=True); p_done.add_argument("--task", required=True)
    subparsers.add_parser("status")
    p_skill = subparsers.add_parser("save-skill"); p_skill.add_argument("--name", required=True); p_skill.add_argument("--title", required=True); p_skill.add_argument("--desc", required=True); p_skill.add_argument("--log", required=True)
